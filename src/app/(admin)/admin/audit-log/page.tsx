...
import { useAuditLog } from "@/lib/admin/auditLog";
...
const COLUMNS: AdminColumn<AuditLogEntry>[] = [
  ...
  { id: "actor", header: "Aktè", csvValue: (r) => `${r.actor_id} (${r.actor_role})`, cell: (r) => (
    <span className="text-sm">{r.actor_id} <span className="text-text-secondary">· {ADMIN_ROLE_LABELS[r.actor_role]}</span></span>
  ) },
...
export default function AuditLogPage() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <AdminPageHeader
        title="Jounal Odit"
        description="Chak aksyon sansib nan admin la anrejistre isit la. Entre yo mockup pandan sesyon an — pa gen tab audit_logs reyèl ankò."
      />
      ...
      <AdminDataTable
        ...
        searchPredicate={(row, q) => row.action.toLowerCase().includes(q) || row.resource_id.toLowerCase().includes(q)}
