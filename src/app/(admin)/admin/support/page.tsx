...
const KANBAN_COLUMNS: { status: AdminSupportStatus; label: string }[] = [
  { status: "new", label: "Nouvo" },
  { status: "in_progress", label: "An Kou" },
  { status: "waiting_customer", label: "Ap Tann Kliyan" },
  ...
  return (
    <Card>
      <CardContent className="flex flex-col gap-2">
        ...
        <span className="text-xs text-text-secondary">{SUPPORT_CATEGORY_LABELS[ticket.category]} · {ticket.assignedAgent}</span>
      </CardContent>
    </Card>
  );
}
...
export default function SupportPage() {
  ...
  const columns: AdminColumn<AdminSupportTicket>[] = [
    ...
    { id: "category", header: "Kategori", csvValue: (r) => SUPPORT_CATEGORY_LABELS[r.category], cell: (r) => SUPPORT_CATEGORY_LABELS[r.category] },
  ...
  const kanbanTickets =
    priorityFilter === "all" ? MOCK_SUPPORT_TICKETS : MOCK_SUPPORT_TICKETS.filter((t) => t.priority === priorityFilter);

  return (
    <div className="flex flex-col gap-6 p-6">
      <AdminPageHeader
        title="Sipò"
        description="Tikè sipò ak entèvansyon — Nouvo → An kou → Ap tann kliyan → Rezoud."
        actions={
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            {KANBAN_COLUMNS.map((col) => {
              const tickets = kanbanTickets.filter((t) => t.status === col.status);
              return (
                <div key={col.status} className="flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-foreground">{col.label}</h3>
                    <span className="text-xs text-text-secondary">{tickets.length}</span>
                  </div>
