...
export function SyncClient({ rows }: { rows: SyncHealthRow[] }) {
  ...
  const offline3d = rows.filter((r) => (daysSince(r.lastSyncAt) ?? Infinity) > 3).length;
  const pendingActions = rows.reduce((sum, r) => sum + r.pendingActions, 0);
  const errorCount = rows.reduce((sum, r) => sum + r.errors, 0);
  ...
  const columns: AdminColumn<SyncHealthRow>[] = [
    ...
    { id: "sync", header: "Dènye Sync", csvValue: (r) => r.lastSyncAt ?? "—", cell: (r) => {
      ...
      return (
        <span className={days !== null && days > 3 ? "font-medium text-danger" : "text-text-secondary"}>
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <AdminDataTable
        ...
        emptyTitle="Pa gen aparèy asiyen pou kounye a"
        emptyDescription="Sante sync parèt isit la yon fwa yon tablèt asiyen a yon boutik e li senkwonize."
      />
